...
def test_toggle():
    ...
    toggle_mode = Toggle_Condition(state = Neomano_state.EXTENDED)
    assert toggle_mode.state == Neomano_state.EXTENDED
...
def test_continuous():
    ...
    assert Neomano_output.FLEX == continuous_mode.process(NexusOutput.MOVE)

    assert Neomano_output.EXTEND == continuous_mode.process(NexusOutput.REST)
